      ...
      <div class="mt-3">
        ...
        <form @submit.prevent="handleSubmit" class="content-spacing">
          <div class="grid-2-cols">
            <div>
              <label class="label-base">标题 *</label>
              <input
                v-model="form.title"
                type="text"
                required
            ...
            <div>
              <label class="label-base">发布时间</label>
          ...
          <div class="grid-2-cols">
            ...
            <!-- 标签选择 -->
            <div>
              <label class="label-base">标签</label>
              <div class="relative">
                <button
                  type="button"
                ...
                >
                  <div
                    ...
                    class="px-3 py-2 hover:bg-gray-50 cursor-pointer text-sm"
                    :class="{
                      'bg-indigo-50 text-indigo-700': selectedTags.some((t) => t.id === tag.id)
                    }"
                    @click="toggleTag(tag)"
                  >
                    {{ tag.name }}
          ...
          <div>
            <label class="label-base">摘要</label>
            <textarea
...
<script setup>
  ...
  const initializeForm = async () => {
    if (props.article) {
      let articleData = props.article

      // 如果是编辑模式，获取完整的文章详情（包含内容）
      if (props.isEdit && props.article.id) {
        try {
          const response = await getArticle(props.article.id)
          if (response.data) {
      ...
      form.value = {
        ...articleData,
        publishTime: formatDateForInput(articleData.publishTime),
      ...
      if (articleData.category) {
        selectedCategory.value = articleData.category
        form.value.categoryId = articleData.category.id
      } else if (articleData.categoryId && categories.value.length > 0) {
        selectedCategory.value =
          categories.value.find((cat) => cat.id === articleData.categoryId) || null
      }

      // 设置选中的标签
      if (articleData.tags && articleData.tags.length > 0) {
        selectedTags.value = articleData.tags
        form.value.tagIds = articleData.tags.map((tag) => tag.id)
      } else if (articleData.tagIds && articleData.tagIds.length > 0 && tags.value.length > 0) {
        selectedTags.value = tags.value.filter((tag) => articleData.tagIds.includes(tag.id))
        form.value.tagIds = articleData.tagIds
      }

      // 如果有封面URL，设置预览
      if (articleData.coverUrl) {
        getFileUrl(articleData.coverUrl)
          .then((url) => {
  ...
  onMounted(async () => {
    ...
    // 并行加载分类标签数据和初始化表单
    await Promise.all([loadCategoriesAndTags(), initializeForm()])
  })
